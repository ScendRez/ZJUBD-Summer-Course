...
public class SystemCameraActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_TAKE_PHOTO && resultCode == RESULT_OK) {
            ...
            //只查询图片信息
            BitmapFactory.decodeFile(takeImagePath,bmoption);
            int photoH=bmoption.outHeight;
            int photoW=bmoption.outWidth;
            int scale=Math.min(photoH/targetH,photoW/targetW);
            bmoption.inJustDecodeBounds=false;
            bmoption.inSampleSize=scale;
            Bitmap bitmap=BitmapFactory.decodeFile(takeImagePath,bmoption);
            imageView.setImageBitmap(bitmap);
            // todo 1.5 注意图片大小
        }
    }
